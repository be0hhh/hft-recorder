...
- **No exceptions.** Library compiled `-fno-exceptions`; recorder follows suit. Every fallible
  operation returns `Status` (see `ApiContracts.md`).
...
- `ApiContracts.md` § `Status` enum — the error surface.
- `ConfigAndCli.md` — signal handling wiring, CPU_CONTROL env var.
- `LoggingAndMetrics.md` — which metrics fire for each incident row.
