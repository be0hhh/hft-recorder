...
live in `src/Runtime/src/Corpus/Codec/ref/*`. They do NOT participate in the recording file format
...
bench tool. They become reference codecs in `src/Runtime/src/Corpus/Codec/ref/pfor_codec.hpp`. TurboPFor we
...
- `CodecVariants.md` — our 7 custom codecs (what we're benchmarking *against* these libs).
- `BENCHMARK_PLAN.md` — 28-cell measurement grid; this doc extends it with external codecs.
- `ComparisonMatrix.md` — earlier research version of the same idea.
- `CustomIdeaCatalog.md` — list of transform ideas; many can use these libraries as components.
- `CodingStyle.md` § E.4 — the spdlog/fmt wrapper rule.
