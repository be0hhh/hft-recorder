...
- **No exceptions, no RTTI** in hot-path code (`src/Runtime/src/Corpus/Codec/`, `src/Runtime/src/Corpus/Block/`, `src/Runtime/src/online/`, any `src/Lab/Variants/*/*/src/Encode.cpp` or `Decode.cpp`). Application-layer code under `src/Process/src/` may use exceptions for startup/CLI errors only.
...
- Direct `#include` of `"network/ws/WsClient.hpp"`, `"parse/..."`, `"exchanges/..."`, `"runtime/..."` — banned. See [BuildAndIsolation.md](BuildAndIsolation.md) for the allowed include surface.

---

## 6. I/O and logging in hot path

- **No `printf`, `std::cout`, `std::cerr`** in `src/Runtime/src/` or variant implementation `src/` directories. Use `spdlog` (from `extra/spdlog`) via the thin wrapper introduced in [LoggingAndMetrics.md](LoggingAndMetrics.md).
...
- **CPU pinning mandatory** for all long-running threads (producer and writer). Use a local helper `src/Runtime/src/Common/ThreadAffinity.hpp` that wraps `sched_setaffinity`. The CXETCPP public header does not expose its internal pinning helper, so a tiny duplicate here is allowed.
- **SPSC rings between producer and writer**: use a lock-free `SpscRing<T, Capacity>` from `src/Runtime/src/Common/`. Do not share mpsc or mpmc queues across stream boundaries.
...
- No `std::shared_ptr` in the hot path. Ownership in `src/Runtime/src/` is through raw owning pointers or `std::unique_ptr` held by a single owner.
...
- **Enum values**: `kAllCaps` for `enum class` entries with a domain prefix — `CodecId::Varint`, `BlockType::CoderReset`. See [ApiContracts.md](ApiContracts.md).
...
// 2. Component-private includes — quoted, relative to the exact owning source root
#include "common/Span.hpp"  // src/Runtime/src
#include "../src/Runtime/src/Corpus/Codec/VarInt.hpp"
...
- Library code returns `bool` or a typed result struct. No exceptions thrown from `src/Runtime/src/`.
- App-layer code in `src/Process/src/` may throw on unrecoverable startup errors (bad CLI, missing `.env`, `libcxet_lib.so` not found). `main` catches and exits with a specific code — see [ConfigAndCli.md](ConfigAndCli.md).
- For per-event failures (malformed event from CXETCPP, SPSC overflow, CRC mismatch on decode), log at `warn` and emit a `GAP_MARKER` block — see [ErrorHandlingAndGaps.md](ErrorHandlingAndGaps.md) for the full table.
...
All of these live in `src/Runtime/src/Common/Constants.hpp` and are the single source
...
- [ ] Round-trip test updated if the wire format changed (see [TestingContract.md](TestingContract.md))
