...
2. `docs/CodecVariants.md` — row `codec_id = LZ4`
3. `docs/ExternalLibraries.md` — lz4 link line, frame vs block mode
4. `docs/Streams.md` — §"L1 / bookTicker"
5. `docs/ComparisonMatrix.md`

## How to implement (Phase 3)

1. Finish `src/Lab/Codecs/src/Lz4Wrapper.cpp` (prefer `LZ4_compress_default`
   for block mode — we manage framing via BlockHeader ourselves).
2. Define `BookTickerRow` POD in `src/RowLayout.hpp`. Use int64 scaled 1e8 for
   prices/qty per `docs/CodingStyle.md`.
3. `src/Encode.cpp`: serialize rows → `lz4Encode` into caller's `out`.
4. `src/Decode.cpp`: `lz4Decode` → split by `sizeof(BookTickerRow)`.
