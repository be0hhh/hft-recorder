...
1. Define `OrderBookUpdateHeader` + `LevelEntry` PODs in `src/RowLayout.hpp`.
2. `src/Encode.cpp`: walk the event stream, emit header then raw `(price, qty)` pairs.
3. `src/Decode.cpp`: inverse.
