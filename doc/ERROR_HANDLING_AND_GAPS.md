...
| 2 | WS disconnect (depth@0ms) | Callback not invoked for > 5 s OR library returns `false` from `runSubscribeOrderBookDeltaByConfig` | Callback thread exits; control thread relaunches the producer after 1 s; writer emits `CODER_RESET` on next block. | `CODER_RESET` + `GAP_MARKER` event |
